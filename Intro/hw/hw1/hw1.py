...
#
# returns the cost of a trip of based on parameters: distance: the trip's
# distance in miles (type: float) vehSpeed: the speed of the vehicle used for
# the trip, in miles per hour (type: float) vehMPG: the vehicle fuel
# efficiency in miles per gallon (type: float) gasCostPerGallon: the price of
# one gallon of gas in dollars (type: float) hotelCostPerNight: the cost of
# one night in a hotel in dollars (type: float)
#
def tripCost(distance: object, vehSpeed: object, vehMPG: object,
             gasCostPerGallon: object,
...
#
# computes (using the tripCost function above)  the cost of a trip for two
# different vehicles, prints information about the cost, and prints a
# recommendation for which vehicle should be used to save money.
#
# The parameters are: distance: the trip's distance in miles (type: float)
# veh1Name: a string representing the first vehicle (e.g. "Tesla") veh1Speed:
# vehicle 1's speed in miles per hour (type: float) veh1MPG: vehicle 1's fuel
# efficiency in miles per gallon (type: float) veh2Name: a string
# representing the second vehicle (e.g. "Dongfend") veh2Speed: vehicle 2's
# speed in miles per hour (type: float) veh2MPG: vehicle 2's fuel efficiency
# in miles per gallon (type: float) gasCostPerGallon: the price of one gallon
# of gas in dollars (type: float) hotelCostPerNight: the cost of one night in
# a hotel in dollars (type: float)
#
# Presume that you can only drive 8 hours per day. So, if a trip requires
# 15.25 hours, it will require a one-night hotel stay at an additional cost (
# beyond gas costs) equal to nightlyHotelCost. (Note: an exact 8-hour trip
# does not require a hotel stay, and a 16-hour trip requires just one night
# of hotel.)
#
