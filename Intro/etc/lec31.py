...
def mergeSort(L):
    ...
    if (len(L) < 2):
        ...
        # mergedL is now sorted but we need to update L itself.
        # L[:] = updates L, replacing each element in L with the corresponding element of mergedL
        L[:] = mergedL[:]
        #print("Merge result:   ", L)
        return
